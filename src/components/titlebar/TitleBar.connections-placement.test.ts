...
describe('TitleBar connection placement', () => {
  // The fork keeps the Connections button in the title bar: each remote opens
  // in a connection window of its own, and the button is how you get there.
  it('keeps the Connections button in the native title bar', () => {
    const source = readFileSync('src/components/titlebar/TitleBar.tsx', 'utf8')

    expect(source).toContain('{native && <RemoteConnectionsDialog />}')
  })
