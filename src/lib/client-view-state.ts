...
import { getActiveConnectionId, isConnectionWindow } from './remote-connections'

export const CLIENT_VIEW_STATE_STORAGE_KEY = 'jean-client-view-state-v1'

/**
 * Every window of the desktop app shares one localStorage. A connection window
 * shows one remote under that remote's own ids, so it keeps a key of its own;
 * sharing the main window's key would let each overwrite the other.
 */
function clientViewStateStorageKey(): string {
  return isConnectionWindow()
    ? `${CLIENT_VIEW_STATE_STORAGE_KEY}:${getActiveConnectionId()}`
...
export function loadStoredClientViewState(): ClientViewState | null {
  ...
  try {
    const stored = globalThis.localStorage.getItem(clientViewStateStorageKey())
