...
export function applyClientViewState(state: ClientViewState): void {
  // Merge rather than replace: pinned sessions and session sorting live in the
  // same per-project settings but persist through the UI-state blob, which is
  // restored first. Replacing would drop them on every start.
  const currentCanvasSettings = useProjectsStore.getState().projectCanvasSettings
      ...
      ...Object.fromEntries(
        Object.entries(state.project_canvas_settings).map(
          ([projectId, settings]) => [
            projectId,
            {
              ...currentCanvasSettings[projectId],
