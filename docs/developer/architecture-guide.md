...
- **MCP** - Model Context Protocol server integration with per-project overrides (`src/services/mcp.ts`). First-party **Jean MCP** (`jean-core/src/jean_mcp_core.rs`) exposes project/worktree/session tools, usage + session model/status controls (`get_usage`, `set_session_model`, `set_session_status` — pins the manual `status_override` so an agent can mark its own session `completed`), Run-command / panel-command dev environments (`get_run_environments`: running state, worktree/base session, startup command, ports, URL), plus the ship loop: `create_commit`, `push_worktree`, `detect_open_pr`, `create_pull_request`, `merge_pull_request`, `run_review` (thin wrappers over existing project commands). Cross-project session questions are answered in one call by `list_all_sessions` (`jean-core/src/chat/overview.rs`) and `search_sessions` (`jean-core/src/chat/search.rs`), so an agent never fans out over `list_projects → list_worktrees → list_sessions`; both filter server-side and enrich (`includeRecap`, `includeOpenWork`) only the page they return, and neither touches the network. The server also sends MCP `instructions` on `initialize` and serves two backend-neutral prompts (`review_sessions_for_archiving`, `find_session`) from `prompts/list` + `prompts/get`, answered locally by the stdio child.
